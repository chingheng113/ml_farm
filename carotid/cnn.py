...
from keras.layers import Dense, Activation, Dropout, Conv1D, MaxPool1D, Flatten, BatchNormalization
...
def cnn(x_data, y_data):
    ...
    nb_neuron_1 = int(round(nb_features*2/3))
    nb_neuron_2 = int(round(nb_neuron_1*2/3))

    early_stop = EarlyStopping(monitor='val_loss', min_delta=0, patience=50, verbose=1, mode='auto')
    callbacks_list = [early_stop]

    model = Sequential()
    model.add(Conv1D(filters=12, kernel_size=2, strides=1, input_shape=(nb_features, 1)))
    model.add(Activation('relu'))
    model.add(MaxPool1D())
    model.add(Dropout(0.3))
    model.add((Conv1D(filters=9, kernel_size=2, strides=1)))
    model.add(Activation('relu'))
    model.add(MaxPool1D())
    model.add(Dropout(0.3))
    # model.add((Conv1D(filters=6, kernel_size=2, strides=1)))
    # model.add(Activation('relu'))
    # model.add(MaxPool1D())
    # model.add(Dropout(0.3))
    model.add(Flatten())
    # model.add(Dense(nb_neuron_1))
    # model.add(Activation('relu'))
    model.add(Dense(nb_neuron_2))
    model.add(Activation('relu'))
    model.add(Dense(nb_classes, activation='softmax'))
    model.compile(optimizer=optimizers.sgd(lr=1e-2), loss=losses.binary_crossentropy, metrics=['accuracy'])
...
x_data_train = np.expand_dims(x_data_train, 2)

x_data_test = data_util.scale(x_data_test)
x_data_test = np.expand_dims(x_data_test, 2)

model, history= cnn(x_data_train, y_data_train)
...
for train, test in LeaveOneOut().split(x_data_all):
    y_train = y_data_all.iloc[train]
    x_data_train = x_data_all.iloc[train]
    x_data_train = np.expand_dims(x_data_train, 2)
    model, history = cnn(x_data_train, y_train)
    x_data_test = x_data_all.iloc[test]
    x_data_test = np.expand_dims(x_data_test, 2)
    loss, acc = model.evaluate(x_data_test, to_categorical(y_data_all.iloc[test], 2))
    y_pred = model.predict(x_data_test)
    one_reslut = y_pred[0]
